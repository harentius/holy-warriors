...
const level0 = {
  ...
  create() {
    ...
    // Init audio track
    const audioTyping = phaserGame.add.audio('typing');
    phaserGame.sound.setDecodedCallback([audioTyping], () => {
      audioTyping.play(null, null, 0.3);
      audioTyping.loopFull(0.3);
    }, phaserGame);

    const audioDoorOpen = phaserGame.add.audio('door-open');

    phaserGame.add.sprite(0, 0, 'frame-1-background');
    const door = phaserGame.add.sprite(20, config.floorPosition - 49, 'door');
